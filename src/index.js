const htmlContent = `<!doctype html><html lang="az"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width, initial-scale=1, viewport-fit=cover"><title>Domino statistika</title>
<style>
...
$('fm').innerHTML='<label>Qalib cütü<select id="wp">'+W.map(p=>pOpt(p,wv)).join('')+'</select></label><label>Uduzan cütü<select id="lp">'+(L.length?L.map(p=>pOpt(p,lv)).join(''):'<option value="">— fərqli qalib cütü seçin —</option>')+'</select></label><label>Nəticə<select id="r"><option>1</option><option>2</option><option>3</option></select></label><label style="flex:1 1 200px">Şərh (könüllü)<textarea id="cm" placeholder="məs: Adil son əlde reş açdı" maxlength="300">'+cmv+'</textarea></label><button id="add">Əlavə et</button>';
$('wp').onchange=()=>frm();
...
$('adm').innerHTML='<p><input id="npName" placeholder="Yeni oyunçu adı"><button id="npAdd">Əlavə et</button>&nbsp; <select id="dp">'+activePlayers().map(p=>'<option value="'+p.id+'">'+p.name+'</option>').join('')+'</select><button id="dpDel">Sil/arxivlə</button>&nbsp; <button id="csv">CSV yüklə</button> <button id="logbtn">Tarixçə</button>&nbsp; <button id="resetBtn" style="border-color:var(--bad);color:var(--bad)">⚠ Hamısını sıfırla</button></p><div class="sc"><table>'+[...ST.matches].slice(-25).reverse().map(m=>'<tr class="'+(m.cancelled?'x':'')+'"><td>'+m.played_at.slice(0,16).replace('T',' ')+'</td><td>'+pname(m.w1)+'–'+pname(m.w2)+'</td><td>'+pname(m.l1)+'–'+pname(m.l2)+'</td><td><select data-r="'+m.id+'"><option '+(m.result==1?'selected':'')+'>1</option><option '+(m.result==2?'selected':'')+'>2</option><option '+(m.result==3?'selected':'')+'>3</option></select></td><td><button data-c="'+m.id+'" data-v="'+(m.cancelled?0:1)+'">'+(m.cancelled?'Bərpa':'Ləğv et')+'</button></td></tr>').join('')+'</table></div><div id="logbox"></div><h3 style="font-size:14px">Toplu idxal (Excel .xlsx və ya CSV)</h3><p class="mut sm">Sütunlar (baş sətirdə, kiçik hərflə): <b>w1,w2,l1,l2,result</b> məcburi, <b>played_at</b> (məs: 2026-01-05 10:00:00) və <b>comment</b> könüllü. w1/w2 = qalib cütü, l1/l2 = məğlub cütü, oyunçu adları mətn kimi.</p><input type="file" id="impFile" accept=".xlsx,.xls,.csv"> <button id="impBtn">İdxal et</button><p id="impStatus" class="sm"></p>';
