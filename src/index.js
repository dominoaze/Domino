...
async function boot(){loginForm();try{const w=await api('/whoami');ROLE=w.role;$('err').textContent='';$('app').hidden=false;await load()}catch(e){$('app').hidden=true;if(GK\vert{}\vert{}AK)$('err').textContent='Giriş açarı yanlışdır.'}}
...
$('npAdd').onclick=async()=>{try{await api('/admin/players',{method:'POST',headers:{'content-type':'application/json'},body:JSON.stringify({name:$('npName').value})});await load()}catch(e){alert(e.msg)}};$('dpDel').onclick=async()=>{try{await api('/admin/players/'+$('dp').value,{method:'DELETE'});await load()}catch(e){alert(e.msg)}};$('csv').onclick=()=>{const h=AK?{'x-admin-key':AK}:{};fetch('/api/admin/export.csv',{headers:h}).then(r=>r.blob()).then(b=>{const a=document.createElement('a');a.href=URL.createObjectURL(b);a.download='domino-oyunlar.csv';a.click()})};
...
$('pf').addEventListener('change',e=>{if(e.target.id=='ps'){PROF=+e.target.value;render()}});$('chips').addEventListener('click',e=>{const k=e.target.dataset.c;if(k){const kk=+k;on.has(kk)?on.delete(kk):on.add(kk);render()}});
